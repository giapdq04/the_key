...
// Glitch characters for the text effect
...
export default function Notfound() {
  ...
  useEffect(() => {
    const update = (e) => {
      setMouseX(e.pageX / window.innerWidth - 0.5); // Giá trị từ -0.5 đến 0.5
      setMouseY(e.pageY / window.innerHeight - 0.5);
    };
  ...
  const handleMouseEnter = () => setIsHoveringMessage(true);
  const handleMouseLeave = () => setIsHoveringMessage(false);

  // Tính góc nghiêng dựa trên vị trí chuột
  const bearRotation = {
    transform: `rotate(${mouseX * 20}deg) translateY(-50%)`, // Nghiêng tối đa 20 độ
  };

  // Generate glitchy characters for each letter
  const generateGlitchyChar = (char, idx) => {
    const charStyle = {
    ...
    return (
      <span
        className={styles.glitchyTextChar}
        aria-hidden={true}
        data-char={char}
        key={`glitch-char--${idx}`}
        style={charStyle}
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.planets}>
        <div className={styles.planetLeft}></div>
        <div className={styles.planetTopRight}></div>
        <div className={styles.planetBottomRight}></div>
      </div>
      ...
      <svg
        className={`${styles.bearLogo} ${styles.bearLogoTears}`}
        style={bearRotation}
        viewBox="0 0 300 300"
      ...
      >
        <g transform="matrix(1.34105 0 0 1.34105 -51.157 -1049.694)">
          ...
          />
          <path
            className={styles.bearTearStream}
          ...
          />
          <circle className={`${styles.bearEye} ${styles.bearEyeLeft}`} r="11.185" cy="894.081" cx="97.242" />
          <circle className={`${styles.bearEye} ${styles.bearEyeRight}`} r="11.185" cx="201.851" cy="894.081" />
          <g className={styles.bearShades}>
      ...
      <h1 className={`${styles.code} ${styles.fourOhFourCode}`}>
        <span className={styles.glitchyTextCharReadable}>404</span>
        {Array.from("404").map((char, idx) => generateGlitchyChar(char, idx))}
      </h1>

      <h2 className={`${styles.codeMessage} ${styles.fourOhFourCodeMessage}`}>
        <span className={styles.glitchyTextCharReadable}>Not found</span>
        {Array.from("Not found").map((char, idx) => generateGlitchyChar(char, idx))}
      </h2>

      <p className={styles.errorDescription}>
        URL nội dung đã bị thay đổi hoặc không còn tồn tại. Nếu bạn đang lưu trữ URL này, hãy thử truy cập lại từ trang chủ thay vì dùng URL đã lưu.
      </p>

      <style jsx>{`
        .${styles.bearLogoTears} {
          --bear-tears: ${isHoveringMessage ? 0 : 0.75};
          --bear-brows: ${isHoveringMessage ? 0 : 1};
          --bear-eyes: 1;
        }
        .${styles.bearEyeLeft}, .${styles.bearEyeRight} {
          transform: scaleY(${isHoveringMessage ? 1 : 0.25});
        }
      `}</style>
    </div>
  );
